...
### 10.19 Heal directo a Neon (post-deploy) + fix de FK en traspasos con almacén codificado

Con el fix de 10.17/10.18 ya desplegado en móviles y en el backend (Northflank + build nueva en Play/TestFlight), se corrió `server/sync_api/bin/heal_neon.dart --apply` (script nuevo, mismo patrón que `merge_categorias_duplicadas.dart`: evento-sourced, id determinístico, re-ejecutable, dry-run por defecto) directamente contra Neon de producción. Resultado: 2 tiendas placeholder desactivadas (`tienda-sync`, `almacen:alm-1`) y 4 grupos de categorías duplicadas fusionados. Hizo falta una segunda pasada porque un dispositivo aún sin un ciclo de sync completo con el código nuevo reinyectó datos viejos entre la primera pasada y la verificación — confirmado limpio en una tercera revisión ~15s después.

**Efecto secundario descubierto en producción**: al desactivar `almacen:alm-1`, un dispositivo reportó `SqfliteFfiException 787 FOREIGN KEY constraint failed` al aplicar un `transferCompleted` histórico (2026-06-30) cuyo `tienda_origen_id` es ese id codificado (`almacen:alm-1`, ver `codificarAlmacenEnTraspaso`/`prefijoAlmacenTraspaso` en `packages/posia_core/lib/src/utils/traspaso_util.dart`). Diagnóstico: `transfers.tienda_origen_id`/`tienda_destino_id` tienen FK dura a `stores(id)` sin importar la codificación, pero el fix de la Fase 2 (10.4) que corrigió `AseguradorPadresFk._asegurarUbicacionTraspaso` para **no** crear una tienda falsa cuando el origen es un almacén, dejó de crear también el stub técnico que satisface ese FK — solo garantiza el almacén real (`asegurarAlmacen`), nunca una fila `stores` con el id literal codificado. Cualquier dispositivo que aplicara por primera vez (pull histórico, instalación nueva) un traspaso con origen/destino de almacén sin haber tenido ya esa fila de antes (heredada del código viejo) iba a fallar igual, con o sin el heal — el heal solo lo hizo visible ahora porque los 18 dispositivos recién están hicieron su primer sync completo con el código nuevo.

Fix: `_asegurarUbicacionTraspaso` (`packages/posia_database/lib/src/utils/asegurador_padres_fk.dart`) ahora, además de garantizar el almacén real, garantiza un stub `stores` **inactivo** (`activa: 0`) con el id codificado literal, solo para satisfacer el FK — nunca aparece como tienda real porque nace inactivo (no depende del auto-saneo para desactivarse después). Prueba nueva: `packages/posia_database/test/traspaso_repository_almacen_fk_test.dart` — guarda un traspaso con origen `almacen:alm-1` sin fila `stores` previa y verifica que no lance excepción, que el stub quede inactivo, y que el almacén real también quede garantizado.

Verificado: `dart analyze` limpio, `flutter test` 83/83 en `posia_database` (82 previos + 1 nuevo), `flutter analyze` limpio en `apps/posia_pos`. Diagnóstico en Neon: solo 2 de 4 eventos de traspaso históricos usan origen/destino de almacén codificado — blast radius bajo, pero el fix es necesario para que **cualquier** dispositivo pueda hacer un pull histórico completo sin crashear. Pendiente: este fix, al ser client-side, requiere un nuevo build/release para tomar efecto — el crash reportado ocurrió en un dispositivo ya en la versión desplegada más reciente.

### 10.20 Promociones: módulo de Lotes de mayoreo + Combos de precio fijo (nuevo)

Requerimiento del usuario: al comprar N piezas de un mismo producto en distintas presentaciones/sabores (ej. "20 sopas La Moderna, cualquier sabor"), debe cobrarse precio de caja; además debe poder armarse un combo de precio fijo al llevar productos **distintos** juntos (ej. shampoo + acondicionador). El usuario recordaba que la importación por lote ya guardaba algo de esto, pero "solo en el campo de notas" — investigación confirmó que ese hack era de un script Python de migración de una sola vez (`scripts/importar_hola2_neon.py`), no del importador Flutter actual: `LotePromocion` (mayoreo cruzado por cantidad sumada, umbral → precio plano) ya existía completo y conectado a `ServicioCaja`/`MotorPrecio`/sync, solo le faltaba un módulo de administración dedicado (antes únicamente se creaba vía importación CSV).

**A. Lotes de promoción — CRUD desde UI + membresía automática por familia.**
- `AdminPromociones` (`packages/posia_database/lib/src/services/admin_promociones.dart`, nuevo dominio) extrae `_guardarLotePromocionImportado`/`_registrarEventoLotePromocion` de `ServicioAdmin` como `registrarLoteDesdeImportacion` (semántica de fusión, usada por la importación masiva) y agrega `guardarLotePromocion`/`eliminarLotePromocion` (semántica de reemplazo, para la UI nueva) + `sugerirMiembrosDeFamilia(productoPadreId)` (arma miembros desde el producto padre + sus `VarianteProducto` activas) + `nombresDeMiembros` (resuelve nombres para mostrar).
- **Hallazgo importante**: las variantes (`product_variants`) son una tabla distinta a `products`, y `lote_promocion_miembros.producto_id` tiene FK dura a `products(id)`. Antes de este fix, agregar el id de una variante como miembro habría disparado `AseguradorPadresFk.asegurarProducto`, que creaba un stub genérico ("Producto", precio 0) — exactamente el tipo de contaminación de catálogo que esta sesión pasó limpiando en Neon (10.17-10.19). Fix: `asegurarProducto` ahora, si el id no existe en `products` pero sí en `product_variants`, refleja los datos reales de la variante (nombre `"{padre} - {variante}"`, precio, código de barras, activo) en vez de un placeholder en blanco — consistente con que el carrito ya trata el id de una variante como si fuera un producto (`ServicioCaja.agregarVariante`).
- UI: `PantallaPromocionesAdmin` (`apps/posia_pos/lib/screens/pantalla_promociones_admin.dart`), pestaña "Lotes de mayoreo" — lista, crear/editar (nombre, piezas mínimas, precio c/u, miembros con botón "Por familia" que auto-sugiere y botón "Agregar" con buscador multi-selección), switch de activo/inactivo (baja lógica).

**B. Combos de precio fijo (nuevo, no existía nada parecido).**
- Modelo `Combo`/`ComboMiembro` (`packages/posia_core/lib/src/models/combo.dart`): a diferencia de un lote (suma cantidades de cualquier miembro, precio unitario plano al llegar al umbral), un combo exige al menos `cantidadRequerida` de **cada** miembro; cada set completo que quepa en el carrito cobra `precioCombo` fijo total en vez de la suma de precios normales.
- Esquema: migración v35 (`SCHEMA_VERSION` 34→35 en `packages/posia_core/lib/src/constants/posia_constants.dart`, `migrarVersion34A35` en `migraciones_esquema.dart`, dispatch de `onUpgrade` en `posia_local_database.dart`) agrega `combos`/`combo_miembros` — mismo patrón que `lotes_promocion`. `ComboRepository` (mirror de `LotePromocionRepository`).
- Sync: evento `comboReplaced` (nuevo valor en `TipoSyncEvento`, contrato en `clasificacion_arquitectura_sync.dart`) con emisor (`AdminEmisorEventosSync.combo`), aplicador remoto (`AplicadorEventosSqlite._aplicarComboRemoto`), proyector Postgres (`_combo` en `proyector_eventos_postgres.dart`) y tablas Postgres (`esquema_pos_postgres.dart`) — mismo patrón evento-sourced que el resto del catálogo.
- **Motor de precios**: a diferencia de lotes (reescribe `precioUnitario` por línea), un combo con precio fijo total sobre productos heterogéneos no tiene un "precio unitario" natural por línea — se resuelve como descuento a nivel ticket. `calcularDescuentoCombos`/`combosAplicadosEnCarrito` (`packages/posia_pricing/lib/src/calculador_combos.dart`) determinan, por cada combo activo, cuántos sets completos caben (mínimo entre miembros de `cantidad_en_carrito / cantidadRequerida`) y el ahorro total. `ServicioCaja` mantiene `_descuentoCombos` (recalculado tras cada mutación del carrito, igual que `_recalcularPreciosLotePromocion` pero sobre **todo** el carrito porque un combo depende de múltiples productos a la vez) y lo resta en `calcularTotalCarrito()`; al cobrar, se funde en el campo existente `Venta.descuentoTicket` (no se agregó columna nueva) para no bifurcar el descuento manual del de combos en el recibo — la transparencia de "qué combo aplicó" la da `obtenerCombosAplicadosEnCarrito()` para la UI del carrito.
- UI: pestaña "Combos" en la misma pantalla — crear/editar (nombre, precio del set, miembros con cantidad requerida por miembro ajustable con +/-).
- Ambas pestañas accesibles desde el menú admin, entrada nueva "Promociones" (sección Catálogo), con permiso propio (`PermisosAdmin.promociones`) asignable a roles personalizados.

Pruebas nuevas: `calculador_combos_test.dart` (9 casos: completar/no completar, múltiples sets, cantidad dispareja entre miembros, `cantidadRequerida` > 1, sin ahorro real, presentación fija excluida, combo inactivo, suma de varios combos), `combo_test.dart` (3 casos end-to-end vía `ServicioCaja`: completar combo descuenta el total, quitar un miembro retira el descuento, cobrar persiste el total y `descuentoTicket` correctos), `admin_promociones_test.dart` (3 casos: sugerencia por familia incluye padre+variantes, lote con miembro de variante no crea placeholder genérico, guardar/eliminar combo con baja lógica que preserva miembros).

Verificado: `dart analyze` limpio en los 5 paquetes tocados (`posia_core`, `posia_database`, `posia_pricing`, `posia_sync`, `server/sync_api`) y `flutter analyze` limpio en `apps/posia_pos`; `flutter test`/`dart test` — 67/67 `posia_core`, 89/89 `posia_database` (83 previos + 6 nuevos), 17/17 `posia_pricing` (8 previos + 9 nuevos), 25/25 `posia_sync`, 7/7 `server/sync_api`.

Pendiente (mismo patrón que fases anteriores): al ser cambios 100% client-side (excepto el esquema/proyector del server, que sí toma efecto en cuanto se despliegue el backend), los 18 dispositivos necesitan la próxima build/release para poder crear o vender combos — el motor de precios en dispositivos viejos simplemente ignora combos que no conoce (no hay riesgo de crash, solo no aplican el descuento hasta actualizar).
