...
class GestorAccesoBiometrico {
  ...
  /// Face ID / huella con fallback al codigo del dispositivo (iOS/Android).
  Future<bool> autenticarDispositivo(String razon) async {
    try {
  ...
  Future<String> etiquetaBiometria() async {
    try {
      ...
      if (tipos.contains(BiometricType.strong) ||
          tipos.contains(BiometricType.weak)) {
        return 'Biometría';
      }
    } on Object {
  ...
  Future<void> registrarPerfil(PerfilAccesoBiometrico perfil) async {
    ...
    final filtrados = todos
        .where((p) => p.usuarioId != perfil.usuarioId)
        .toList();
  ...
  Future<PerfilAccesoBiometrico?> autenticarYRecuperar({
    ...
    final ok = await autenticarDispositivo('Iniciar sesión como ${perfil.nombre}');
    if (!ok) {
      return null;
    }
    return perfil;
  ...
  Future<List<PerfilAccesoBiometrico>> _leerTodos() async {
    ...
    try {
      ...
      return lista
          .map(
            (e) => PerfilAccesoBiometrico.fromJson(e as Map<String, dynamic>),
          )
          .toList();
    } on Object {
      return [];
