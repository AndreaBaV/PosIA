...
class AnalisisImportacionProductos {
	const AnalisisImportacionProductos({
		required this.filas,
		this.errorArchivo,
	});
...
class ImportadorProductos {
	...
	static const _aliasColumnas = <String, List<String>>{
		...
		'categoria': ['categoria', 'categoría', 'category', 'departamento', 'rubro'],
		...
		try {
			...
			if (!mapaColumnas.containsKey('precio_base')) {
				return const AnalisisImportacionProductos(
					filas: [],
					errorArchivo:
						'Falta la columna "precio_base". Descargue la plantilla e intente de nuevo.',
				);
			}
			if (!mapaColumnas.containsKey('categoria')) {
				return const AnalisisImportacionProductos(
					filas: [],
					errorArchivo:
						'Falta la columna "categoria". Descargue la plantilla e intente de nuevo.',
				);
			}
			...
			for (var i = 1; i < filasCrudas.length; i++) {
				...
				if (categoriaTexto.isEmpty) {
					...
					final categoria = categoriasPorNombre[
						normalizarTextoBusqueda(categoriaTexto)
					];
					if (categoria == null) {
						errores.add('Categoria no encontrada: "$categoriaTexto"');
					} else {
				...
				if (codigoBarras.isNotEmpty) {
					...
					if (codigosBarrasExistentes.contains(codigoNorm)) {
						errores.add('Codigo de barras ya existe en el catalogo: $codigoBarras');
					}
					if (codigosEnArchivo.contains(codigoNorm)) {
						errores.add('Codigo de barras duplicado en el archivo: $codigoBarras');
					} else {
				...
				if (proveedorTexto.isNotEmpty) {
					final proveedor = proveedoresPorNombre[
						normalizarTextoBusqueda(proveedorTexto)
					];
					if (proveedor == null) {
	...
	static List<List<String>> _leerFilas(Uint8List bytes, String extension) {
		...
		throw FormatException(
			'Formato no soportado (.$ext). Use .csv o .xlsx',
		);
	}
