...
void main() {
    ...
    final cliente = HubSyncClient(
      ...
      clienteHttp: MockClient((request) async {
        ...
        if (request.method == 'POST' &&
            request.url.path.endsWith('/v1/events')) {
  ...
  test(
    'push usa storeId del evento si el orquestador tiene tienda vacia',
    () async {
      final cola = ColaEventosMemoria();
      ...
      final cliente = HubSyncClient(
        ...
        clienteHttp: MockClient((request) async {
          ...
          if (request.method == 'POST') {
            final cuerpo = jsonDecode(request.body) as Map<String, Object?>;
  ...
  test(
    'sincronizarDesdeOrigen NO excluye el dispositivo propio en el pull',
    () async {
      final cola = ColaEventosMemoria();
  ...
  test(
    'sincronizarDesdeOrigen no se engancha a un ciclo incremental',
    () async {
      final cursor = CursorMemoria()..cursor = 90;
      final peticiones = <Uri>[];
      final cliente = HubSyncClient(
  ...
  test(
    'ciclos simultaneos se enganchan al mismo sync sin duplicar POST',
    () async {
      final cola = ColaEventosMemoria();
