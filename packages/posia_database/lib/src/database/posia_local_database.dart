...
class PosiaLocalDatabase {
		...
		try {
			final db = await openDatabase(
				...
				onUpgrade: (base, anterior, nueva) async {
					...
					if (anterior < 37 && nueva >= 37) {
						await MigracionesEsquema.migrarVersion36A37(base);
					}
					if (anterior < 38 && nueva >= 38) {
						await MigracionesEsquema.migrarVersion37A38(base);
					}
					if (anterior < 39 && nueva >= 39) {
	...
	Future<void> _migrarEsquemaTenant(
		...
		// v33/v34 se aplican en fase 2 (openDatabase version SCHEMA_VERSION).
	}
}
