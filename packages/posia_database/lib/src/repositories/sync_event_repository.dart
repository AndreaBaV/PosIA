...
class SyncEventRepository implements LocalEventQueue {
  ...
  Future<void> encolar(SyncEvent evento) async {
    await _baseDatos.insert('sync_event_queue', {
      'id': evento.id,
  ...
  static final String _listaTipoPrioridadSql = _tiposPrioridadProducto
      .map((tipo) => "'$tipo'")
      .join(',');

  @override
  Future<List<SyncEvent>> obtenerPendientes() async {
    // Lectura en escritura: tras encolar + empujar al instante, el snapshot
    // de la conexion read-only puede no ver aun la fila (asistencia, etc.).
    final filas = await _consultarCola(
      where: 'estado = ? OR estado = ?',
      whereArgs: [EstadoSyncEvento.pendiente.name, EstadoSyncEvento.error.name],
      orderBy:
          'CASE WHEN tipo IN ($_listaTipoPrioridadSql) THEN 0 ELSE 1 '
          'END ASC, creado_en ASC',
    );
    return filas.map(_mapearEvento).toList();
  }

  Future<List<Map<String, Object?>>> _consultarCola({
    String? where,
    List<Object?>? whereArgs,
    String? orderBy,
  }) async {
    final base = _baseDatos;
    if (base is ConexionOperativaRuteada) {
  ...
  Future<int> contarPendientes() async {
    final filas = await _baseDatos.rawQuery(
      '''
			...
			WHERE estado IN (?, ?)
		''',
      [EstadoSyncEvento.pendiente.name, EstadoSyncEvento.error.name],
    );
    ...
    try {
      ...
      switch (tipo) {
        ...
        case 'priceListItemDeleted':
          final lista =
              payload['listaPreciosId']?.toString() ??
  ...
  SyncEvent _mapearEvento(Map<String, Object?> fila) {
    final payloadJson =
        jsonDecode(fila['payload'] as String) as Map<String, Object?>;
    return SyncEvent(
