...
class LotePromocionRepository {
  ...
  /// Lista todos los lotes (activos e inactivos) con sus miembros.
  Future<List<LotePromocion>> listarTodos({DatabaseExecutor? db}) async {
  ...
  Future<LotePromocion?> obtenerPorCodigoExterno(
    ...
    return _mapear(
      filas.first,
      await _miembrosDe(filas.first['id']! as String, db: exec),
    );
  }
  ...
  Future<void> reemplazarLote(
    ...
    await ejecutarEscrituraTransaccional(_baseDatos, db, (tx) async {
      await tx.insert('lotes_promocion', {
        'id': lote.id,
      ...
      for (final productoId in lote.productoIds.toSet()) {
        ...
        await tx.insert('lote_promocion_miembros', {
          'lote_id': lote.id,
  ...
  Future<void> agregarMiembro(
    ...
    final exec = db ?? _baseDatos;
    await exec.insert('lote_promocion_miembros', {
      'lote_id': loteId,
      'producto_id': productoId,
    }, conflictAlgorithm: ConflictAlgorithm.replace);
  }
