...
class TraspasoRepository {
		...
		await ejecutarEscrituraTransaccional(_baseDatos, db, (tx) async {
			await tx.insert(
				'transfers',
				{
					...
					'almacen_origen_id': traspaso.almacenOrigenId,
					'almacen_destino_id': traspaso.almacenDestinoId,
	...
	Future<Traspaso> _mapear(Map<String, Object?> fila) async {
		...
		return Traspaso(
			...
			almacenOrigenId: fila['almacen_origen_id'] as String? ?? '',
			almacenDestinoId: fila['almacen_destino_id'] as String? ?? '',
