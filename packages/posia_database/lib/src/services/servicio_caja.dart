...
class ServicioCaja {
	...
	Future<List<Producto>> listarProductos({String? categoriaId}) async {
		...
		return _productoRepository.listarActivosPorCategoria(_tiendaId, categoriaId);
	}
	...
	Future<String?> validarCobro() async {
		...
		if (corte != null && !await corte.tieneTurnoAbierto()) {
			await corte.abrirTurno(fondoInicial: 0.0, vendedorId: _vendedorActivo?.id);
		}
		return null;
	}
	...
	Future<void> agregarProducto(Producto producto, {double cantidad = 1.0}) async {
	...
	Future<String> agregarProductoConPeso(Producto producto, double pesoKg) async {
		final servicioCarniceria = _servicioCarniceria;
	...
	Future<String> agregarProductoConLote(
		...
		final validacion = await servicioFarmacia.validarLoteParaVenta(lote.id, cantidad);
	...
	Future<List<VarianteProducto>> listarVariantesActivas(String productoId) async {
	...
	Future<bool> agregarPorCodigoBarras(String codigoBarras) async {
		final variante = await _varianteRepository?.buscarPorCodigoBarras(codigoBarras);
		if (variante != null) {
			await agregarVariante(variante);
			return true;
		}
		final producto = await _productoRepository.buscarPorCodigoBarras(codigoBarras);
		if (producto == null) {
			return false;
		}
	...
	Future<void> _agregarLineaCarrito({
		...
		if (permitirFusion) {
			...
			if (indiceExistente >= 0) {
				...
				final precioActualizado = await _motorPrecio.resolverPrecio(contextoActualizado);
